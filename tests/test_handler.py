...
def test_reload_control_command():
    class _M:
        ...
        async def reload_listener(self, account=None):
            self.reloaded = account

        async def get(self, account=None):
            raise AssertionError("not a telegram op")
...
def test_stt_reload_control_command(monkeypatch):
    ...
    monkeypatch.setattr(
        stt, "reset_models", lambda: calls.__setitem__("reset", calls["reset"] + 1)
    )
    ...
    out = asyncio.run(
        handler.handle_request(_M(), {"id": 5, "cmd": "stt_reload", "tier": "read"})
    )
...
def test_stt_status_control_command(monkeypatch):
    ...
    monkeypatch.setattr(
        local, "_models", {("faster-whisper", "tiny", "cpu", "int8"): object()}
    )
    ...
    out = asyncio.run(
        handler.handle_request(_M(), {"id": 6, "cmd": "stt_status", "tier": "read"})
    )
    assert out["ok"]
