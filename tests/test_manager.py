...
def test_manager_connects_once_and_caches(monkeypatch):
    ...
    monkeypatch.setattr(
        "tlgrm.accounts.resolve_account", lambda name=None: name or "default"
    )
...
def test_manager_get_unauthorized_raises(monkeypatch):
    ...
    monkeypatch.setattr(
        cc, "get_client", lambda account=None, must_exist=True: _Unauth()
    )
    monkeypatch.setattr(
        "tlgrm.accounts.resolve_account", lambda name=None: name or "default"
    )
...
def test_manager_starts_and_reloads_listener(monkeypatch):
    ...
    monkeypatch.setattr(
        cc, "get_client", lambda account=None, must_exist=True: _FakeClient()
    )
    monkeypatch.setattr(
        "tlgrm.accounts.resolve_account", lambda name=None: name or "default"
    )
    monkeypatch.setattr(
        "tlgrm.accounts.listen_config",
        lambda name: {
            "enabled": True,
            "webhook_url": None,
            "webhook_headers": [],
